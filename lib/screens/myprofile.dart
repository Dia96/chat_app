...
enum Gender{Male, Female, Trans, NIL}
...
class _MyProfileState extends State<MyProfile> {
 ...
 /** Gender _selected = Gender.Male;
  String _value ='';
      List<PopupMenuEntry<Gender>>
      _items = new
          List<PopupMenuEntry<Gender>>();

      void initState() {
        for(Gender gender in
        Gender.values)
          {
            _items.add(new PopupMenuItem(
              child: new Text(_getDisplay(gender))
                  value: gender
            ));
          }
      }
  ...
  void _onSelected(Gender gender) {
    setState((){
      _selected = gender;
      _value = '${_getDisplay(gender)}';
    });
  }

  String _getDisplay(Gender gender) {
    int index = gender.toString().indexOf('.');
    index++;
    return gender.toString().substring(index);
  }
            ...
            SliverList(delegate: SliverChildListDelegate(<Widget>[
              ...
              Slidable(
                ...
                child: Container(
                    ...
                    child: ListTile(trailing: Icon(Icons.slideshow),
                      leading: Icon(FontAwesomeIcons.slidersH),
                      title: Text('User Id',style: TextStyle(fontSize: 18.0, fontWeight: FontWeight.bold ),
                        textAlign: TextAlign.center,),
                      subtitle: Text(name
